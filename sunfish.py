...
def api_move(move_index):
    ox, oy = board.index_to_chess_notation((move_index[0], move_index[1]))
    tx, ty = board.index_to_chess_notation((move_index[2], move_index[3]))
    entry = ox+oy+tx+ty

    # print_pos(ai.hist[-1])

    if ai.hist[-1].score <= -MATE_LOWER:
        print("You lost")
        return

    # We query the user until she enters a (pseudo) legal move.
    move = None
    while move not in ai.hist[-1].gen_moves():
        match = re.match('([a-h][1-8])'*2, entry)
        ...
        if match:
            ...
            # Inform the user when invalid input (e.g. "help") is entered
            print("Please enter a move like g8f6")
    ai.hist.append(ai.hist[-1].move(move))

    # After our move we rotate the board and print it again.
    # This allows us to see the effect of our move.

    # print_pos(ai.hist[-1].rotate())


def ai_make_move(brd, player):
    if ai.hist[-1].score <= -MATE_LOWER:
        print("You won")

    # Fire up the engine to look for a move.
    start = time.time()
    for _depth, move, score in ai.searcher.search(ai.hist[-1], ai.hist):
        if time.time() - start > 1:
            break
    ...
    ai_move = render(119-move[0]) + render(119-move[1])

    ox, oy = board.convert_to_index(ai_move[0] + ai_move[1])
    tx, ty = board.convert_to_index(ai_move[2] + ai_move[3])
    brd.events.move([ox, oy], [tx, ty], player)
